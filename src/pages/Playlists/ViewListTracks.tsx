...
const ViewListTracks = () => {
  ...
  useEffect(() => {
    const getUserPlaylist = async () => {
      try {
        const response = await axios.get(`https://musicserver-uluy.onrender.com/playlist/listTracks/${id}`);
        setAllSongs(response.data.playlistTracks)
      } catch (error) {
  ...
  const handleDelete = async(id:string)=>{
    try {
      await axios.delete(`https://musicserver-uluy.onrender.com/play-list-track/${id}`);
  ...
  return (
    <div className="p-6 bg-white rounded-lg">
      <h2 className="text-2xl font-semibold mb-4 text-gray-800 text-center">All playlist Tracks</h2>
